...
#import sktime.classification.compose._ensemble as sktime_ens
import sktime.classification.distance_based._elastic_ensemble as sktime_ee
import sktime.classification.distance_based._proximity_forest as sktime_pf
import sktime.classification.interval_based._rise as sktime_rise
import sktime.classification.interval_based._tsf as sktime_tsf
import sktime.classification.dictionary_based._cboss as sktime_cboss
import sktime.classification.dictionary_based._weasel as sktime_weasel
import sktime.classification.distance_based._time_series_neighbors as sktime_knn
...
from sktime.transformations.panel.reduce import Tabularizer as Tabulariser
from sktime.transformations.panel.compose import SeriesToSeriesRowTransformer as RowwiseTransformer
...
class TimeSeriesForestClassifier(_BaseWrapper):
    ...
    def __init__(self, **kwargs):
        super().__init__(sktime_tsf.TimeSeriesForest(**kwargs), encode_sktime_X)
...
class RISEClassifier(_BaseWrapper):
    ...
    def __init__(self, **kwargs):
        super().__init__(sktime_rise.RandomIntervalSpectralForest(**kwargs), encode_sktime_X)
...
class ContractableBOSSClassifier(_BaseWrapper):

    def __init__(self, **kwargs):
        super().__init__(sktime_cboss.ContractableBOSS(**kwargs), encode_sktime_X)



class SktimeWEASELClassifier(_BaseWrapper):

    def __init__(self, **kwargs):
        super().__init__(sktime_weasel.WEASEL(**kwargs), encode_sktime_X)


class SktimeKNNClassifier(_BaseWrapper):

    def __init__(self, **kwargs):
        super().__init__(sktime_knn.KNeighborsTimeSeriesClassifier(**kwargs), encode_sktime_X)
